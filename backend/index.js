...
app.post("/api/admin/update-rate", async (req, res) => {
    try {
        ...
        if(!rate) {
            const newRate = new Rate({ type, rate: new_rate });
            await newRate.save();
            return res.status(201).json({ message: "Gold rate added successfully", rate: newRate });
        }
        rate.rate = new_rate;
        await rate.save();

        res.status(201).json({ message: "Gold rate updated successfully", rate });
    }
    catch(err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/admin/get-rate", async (req, res) => {
    try {
        const gold_rate = await Rate.findOne({ type: "gold" });
        const silver_rate = await Rate.findOne({ type: "silver" });
...
app.post("/api/admin/add-dealer", async (req, res) => {
    try {
        ...
        const id = uuidv4();
        db.run("INSERT INTO dealers (id, name, address, phone) VALUES (?, ?, ?, ?)", [id, name, address, phone], (err) => {
            if(err) {
                return res.status(500).json({ error: err.message });
            }
            res.status(201).json({ message: "Dealer added successfully", dealer: { id, name, address, phone } });
        });
    }
    catch(err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/admin/add-transaction", (req, res) => {
    ...
    if (!dealer_id || !type || !gram || !description) {
        return res.status(400).json({ error: "All fields are required" });
    }

    if (!["DEBT", "CREDIT"].includes(type)) {
        return res.status(400).json({ error: "Type must be DEBT or CREDIT" });
    }

    if (gram < 0) {
        return res.status(400).json({ error: "Gram must be positive" });
    }
    ...
    const query = `
        ...
        (id, dealer_id, type, gram, description, date, time)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;

    db.run(
        query,
        [id, dealer_id, type, gram, description, now, now],
        function (err) {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
...
app.post("/api/admin/update-transaction", async (req, res) => {
    try {
        ...
        if(!dealer_id || !transaction_id || !type || !gram || !description) {
            return res.status(400).json({ error: "All fields are required" });
        }
        if(!["DEBT", "CREDIT"].includes(type)) {
            return res.status(400).json({ error: "Type must be DEBT or CREDIT" });
        ...
        const query = `
            ...
            SET dealer_id = ?, type = ?, gram = ?, description = ?, date = ?, time = ?
            WHERE id = ?
        `;
        db.run(query, [dealer_id, type, gram, description, now, now, transaction_id], function(err) {
            if(err) {
                return res.status(500).json({ error: err.message });
            }
            res.status(200).json({ message: "Transaction updated successfully", transaction: { id: transaction_id, dealer_id, type, gram, description, date: now } });
        });
    }
    catch(err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/admin/expenses", async (req, res) => {
    try {
        ...
        const query = `
            ...
            VALUES (?, ?, ?, ?, ?)
        `;
        db.run(query, [id, amount, description, now, now], function(err) {
            if(err) {
                return res.status(500).json({ error: err.message });
            }
            res.status(200).json({ message: "Expense added successfully", expense: { id, amount, description, date: now } });
        });
    }
    catch(err) {
        res.status(500).json({ error: err.message });
...
app.post("/api/admin/chitfunds/create-chit", async (req, res) => {
    try {
        const { user_id, chitfund_name, chitfund_amount, chitfund_duration, chitfund_type } = req.body;
        if(!user_id || !chitfund_name || !chitfund_amount || !chitfund_type) {
            return res.json({ error: "Fill all mandatory fields" });
        }
...
app.post("/api/admin/chitfunds/add-payment", async (req, res) => {
    try {
        const { user_id, chitfund_id, amount } = req.body;
        if(!user_id || !chitfund_id || !amount) {
            return res.status(400).json({ error: "All fields are required" });
        }
        ...
        if(amount !== chitfund.chitfund_amount) {
