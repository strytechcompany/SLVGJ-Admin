...
export default function ChitFunds({ pink, lightPink, softPink }) {
  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      {selectedUser && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, pointerEvents: (showCreateChitModal || showAddPaymentModal) ? 'none' : 'auto' }} onClick={closeUserModal}>
          <div style={{ background: 'white', borderRadius: '24px', width: '100%', maxWidth: '1100px', maxHeight: '92vh', overflow: 'auto', pointerEvents: 'auto' }} onClick={e => e.stopPropagation()}>
                ...
                userChitfunds.map(chit => (
                  <div
                    ...
                    style={{
                      ...
                      marginBottom: '16px', cursor: chit.status === "ACCEPTED" ? 'pointer' : 'default',
                      transition: 'all 0.2s'
                    }}
      ...
      {/* ADD PAYMENT MODAL */}
      {showAddPaymentModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20000, pointerEvents: 'auto' }} onClick={() => setShowAddPaymentModal(false)}>
          <div style={{ background: 'white', borderRadius: '24px', width: '420px', padding: '32px', pointerEvents: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddPaymentSubmit} style={{ pointerEvents: 'auto' }}>
              <div style={{ marginBottom: '24px' }}>
                ...
                <input type="number" value={addPaymentForm.amount} onChange={e => setAddPaymentForm({amount: e.target.value})} required style={{ width: '100%', padding: '14px', border: `2px solid ${softPink}`, borderRadius: '12px', background: 'white', color: '#1f2937', fontSize: '18px', pointerEvents: 'auto' }} />
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
