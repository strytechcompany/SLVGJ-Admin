...
export default function Dealers({ pink, lightPink, softPink }) {
  ...
  const fetchRatesAndLiabilities = async (dealersList = []) => {
    try {
      ...
      if (rateRes.ok) {
        ...
        liveGoldRate = rates.gold_rate?.rate ?? 6245;
  ...
  const fetchAllDealers = async () => {
    ...
    try {
      ...
      const allDealers = (data.dealers || []).map(d => ({
        ...d,
        dealer_id: d.id
      }));
  ...
  const fetchDealerTransactions = async (dealerId) => {
    try {
      ...
      if (!response.ok) throw new Error();
      const data = await response.json();
      const allTrans = (data.transactions || []).map(t => ({
        ...t,
        transaction_id: t.id
      }));
  ...
  const handleUpdateTransactionSubmit = async (e) => {
    e.preventDefault();
    if (!currentTransaction || !selectedDealer) return;
    const payload = {
