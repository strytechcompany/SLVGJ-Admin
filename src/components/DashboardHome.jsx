...
export default function DashboardHome({ pink, lightPink, softPink }) {
  ...
  const maxRevenue = Math.max(...staticWeeklyData.map(d => d.revenue));
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Today's Sales
        if (salesRes.ok) {
          const { todaysSales: sales } = await salesRes.json();
          setTodaySales(sales || 480000);
        }

        // Weekly Data
        if (weeklyRes.ok) {
          const { weeklyData: week } = await weeklyRes.json();
          if (week?.length) setWeeklyData(week);
        }

        // Recent Transactions
        if (recentRes.ok) {
          const { recentTransactions: rec } = await recentRes.json();
          if (rec?.length) setRecentTransactions(rec);
        }
      } catch (err) {
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: '48px', fontWeight: '700', marginTop: '12px' }}>₹4.8L</div>
